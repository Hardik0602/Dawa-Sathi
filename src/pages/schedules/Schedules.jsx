...
const Time = ({ photo, time, time_of_day, onPress, isVisible, expand, selectTime }) => {
    ...
    return (
        <TouchableWithoutFeedback onPress={onPress}>
            <View className={`bg-[#FFFFFF] ${expand ? 'h-[250px]' : 'h-[70px]'} w-[95%] mb-5 shadow-md shadow-black justify-center items-center flex-col rounded-xl`}>
                <View className='flex-row p-1 h-[70px]'>
                    <View className='flex-col justify-center items-center w-[30%] p-1'>
                        <Text className='text-[20px] font-semibold text-[#A531E9]'>{time_of_day}</Text>
                        <Image
                            source={`${photo}`}
                            className='h-[70%] w-50%]' />
                    </View>
                ...
                {expand ?
                    <View className='mt-10 w-[90%] items-center'>
...
const Schedules = () => {
    ...
    return (
        <View className='flex-1 items-center bg-white justify-center'>
            <View className='absolute top-0 bg-[#A531E9] w-[130%] h-[35%] rounded-b-[300px]' />
            <View>
                <Text className='text-white font-bold text-[35px] mb-3'>Schedules</Text>
            </View>
            <View className='bg-[#FAF9F6] w-[80%] h-[300px] items-center justify-center rounded-3xl shadow-xl shadow-black'>
                <View className='bg-white w-[93%] h-[93%] rounded-3xl p-4 items-center'>
                    ...
                    <View className="flex-row w-full justify-between px-1 mb-2">
                        {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day) => (
                            <Text key={day} className="text-xs font-bold text-[#B0B0B0] w-[13%] text-center">{day}</Text>
                        ))}
                    </View>
                        ...
                        <View key={idx} className="flex-row w-full justify-between mb-1 px-1">
                            {week.map((day) => (
                                <View key={day.date} className={`items-center w-[13%] py-1 rounded-full ${day.isToday ? 'bg-[#AB33ED]' : 'bg-transparent'}`}>
                                    <Text className={`text-sm font-bold ${day.isToday ? 'text-white' : day.isCurrentMonth ? 'text-[#171D1B]' : 'text-[#D2C9DE]'}`}>{day.number}</Text>
                                </View>
                            ))}
                        </View>
                    ))}
            ...
            <View className='w-[90%] items-center mb-10'>
                <Text className='text-[#9838CD] font-bold text-[20px] my-3'>Today's Schedules</Text>
...
// OLD CODE FOR FUTURE API INTEGRATIONS
// import React, { useEffect, useState } from 'react';
// import { View, Text, Image, TouchableWithoutFeedback, TouchableOpacity } from 'react-native';
// import moment from 'moment';
// import morning from '../../../images/morning.png';
// import afternoon from '../../../images/afternoon.png';
// import evening from '../../../images/evening.png';
// const Time = ({ photo, hour, minute, am_pm, time_of_day, onPress, isVisible, expanded }) => {
//     if (!isVisible)
//         return null;
//     return (
//         <TouchableWithoutFeedback onPress={onPress}>
//             <View className={`bg-[#FAF9F6] ${expanded ? 'h-[250px]' : 'h-[70px]'} w-[95%] mb-5 shadow-md shadow-black justify-center items-center flex-col rounded-xl`}>
//                 <View className='flex-row p-1 h-[70px]'>
//                     <View className='flex-col justify-center items-center w-[30%] p-1'>
//                         <Text className='text-[20px] font-semibold text-[#A531E9]'>{time_of_day}</Text>
//                         <Image
...
//                         <TouchableOpacity className='bg-[#9838CD] w-[80%] h-[55px] items-center justify-center rounded-3xl'>
//                             <Text className='text-[30px] text-white font-bold'>Select Time</Text>
//                         </TouchableOpacity>
//                     </View>
//                     : null}
//             </View>
...
//     return (
//         <View className='flex-1 items-center bg-white justify-center'>
//             <View className='absolute top-0 bg-[#A531E9] w-[130%] h-[35%]' style={{ borderBottomLeftRadius: 300, borderBottomRightRadius: 300 }} />
//             <View>
//                 <Text className='text-white font-bold text-[35px] mb-3'>Schedules</Text>
//             </View>
//             <View className='bg-[#FAF9F6] w-[80%] h-[300px] items-center justify-center rounded-3xl shadow-xl shadow-black'>
...
//             <View className='w-[90%] items-center mb-10'>
//                 <Text className='text-[#9838CD] font-bold text-[20px] my-3'>Today's Schedules</Text>
//                 <Time time_of_day={'Morning'} photo={morning} am_pm={'AM'} hour={'05'} minute={'30'} onPress={() => toggleTime('Morning')} expanded={activeTime === 'Morning'} isVisible={activeTime === null || activeTime === 'Morning'} />
//                 <Time time_of_day={'Afternoon'} photo={afternoon} am_pm={'PM'} hour={'12'} minute={'30'} onPress={() => toggleTime('Afternoon')} expanded={activeTime === 'Afternoon'} isVisible={activeTime === null || activeTime === 'Afternoon'} />
//                 <Time time_of_day={'Evening'} photo={evening} am_pm={'PM'} hour={'07'} minute={'30'} onPress={() => toggleTime('Evening')} expanded={activeTime === 'Evening'} isVisible={activeTime === null || activeTime === 'Evening'} />
//             </View>
//         </View>
//     );
// };
